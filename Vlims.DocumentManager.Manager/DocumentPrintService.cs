    ...
    public bool SaveDocumentPrint(DocumentPrint documentPrint)
    {
        try
        {
            ...
            if (validationMessages.Length <= 0)
            {
                ...
                AuditLog.SaveAuditLog(new AuditLogEntity { UserName = documentPrint.CreatedBy, EntityName = documentPrint.DocumentNumber, Type = DocumentPrintConstants.PrintType, state = DefinitionStatus.New, EntityInfo = documentPrint, Unique =documentPrint.documenttitle, RevisionNumber = documentPrint.RevisionNumber });
                return result;
            }
