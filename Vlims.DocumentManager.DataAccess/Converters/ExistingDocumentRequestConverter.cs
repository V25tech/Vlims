            ...
            try
            {
                ...
                if (dataset != null && dataset.Tables.Count > 0 && dataset.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; (i < dataset.Tables[0].Rows.Count); i++)
                    {
                        ...
                        existingDocumentRequestData.EDRId = Convert.ToString(row[ExistingDocumentRequestConstants.EDRId.Trim('@')]);
                        existingDocumentRequestData.Documentmanagerid = Convert.ToString(row[ExistingDocumentRequestConstants.Documentmanagerid.Trim('@')]);
                        ...
                        existingDocumentRequestData.printtype = Convert.ToString(row[ExistingDocumentRequestConstants.printtype.Trim('@')]);
                        existingDocumentRequestData.noofcopies = Convert.ToString(row[ExistingDocumentRequestConstants.noofcopies.Trim('@')]);
                        existingDocumentRequestData.workflow = Convert.ToString(row[ExistingDocumentRequestConstants.workflow.Trim('@')]);
                        existingDocumentRequestData.reason = Convert.ToString(row[ExistingDocumentRequestConstants.reason.Trim('@')]);
                        existingDocumentRequestData.browse = Convert.ToString(row[ExistingDocumentRequestConstants.browse.Trim('@')]);
