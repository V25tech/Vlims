...
BEGIN
UPDATE workitems_PSY SET Status_PSY='Reviewed', Stage_PSY=@Status_PSY,IsCompleted_PSY=1,ModifiedDate_PSY=GETDATE() WHERE RefrenceGuid_PSY=@REFERENCEGUID_PSY AND InitiatedBy_PSY=@UserName
END
ELSE IF(@TYPE='APPROVE' OR @TYPE='APPROVED')
BEGIN
UPDATE workitems_PSY SET Status_PSY='Approved', Stage_PSY=@Status_PSY,IsCompleted_PSY=1,ModifiedDate_PSY=GETDATE() WHERE RefrenceGuid_PSY=@REFERENCEGUID_PSY AND InitiatedBy_PSY=@UserName
END
END
