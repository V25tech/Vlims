CREATE PROCEDURE [dbo].[USP_GetTemplateHeaderFooterDetails_1]
...
(SELECT STRING_AGG(FORMAT(REVIEWDATE,'dd/MM/yyyy HH:mm:ss'),',') FROM @REVIEWERS) AS REVIEWDATES,
...
(SELECT STRING_AGG(FORMAT(APPROVEDATE,'dd/MM/yyyy HH:mm:ss'),',') FROM @APPROVERS) AS APPROVEDATES,
...
(SELECT STRING_AGG(FORMAT(PREPAREDDATE,'dd/MM/yyyy HH:mm:ss'),',') FROM @PREPAREDBY) AS PREPAREDDATES,
