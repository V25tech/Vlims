...
@ModifiedBy_PSY,
 ...
 SELECT @documenttype_PSY,'Request','Pending',@AssigntoGroup_PSY,@CreatedBy_PSY,GetDate(),@Status_PSY,GetDate(),@ID,WSR.Type,0,WSR.UserName,@PARENTGUID_PSY,GETDATE(),WSR.UserName, GETDATE(),@PARENTGUID_PSY from WorkflowUsersMapping WSR WHERE WSR.WorkFlowName=@Workflow_PSY AND WSR.Type='Review'

 INSERT into workitems_PSY(TaskName_PSY,TaskType_PSY,Stage_PSY,AssignedToGroup_PSY,InitiatedBy_PSY,InitiatedOn_PSY,Status_PSY,DueDate_PSY,RefrenceId_PSY,ActionType_PSY,IsCompleted_PSY,CreatedBy_PSY,RefrenceGuid_PSY, CreatedDate_PSY, ModifiedBy_PSY,ModifiedDate_PSY,RefrenceGuid_PSY)
 SELECT @documenttype_PSY,'Request','Pending',@AssigntoGroup_PSY,@CreatedBy_PSY,GetDate(),@Status_PSY,GetDate(),@ID,WSR.Type,0,WSR.UserName,@PARENTGUID_PSY,GETDATE(),WSR.UserName, GETDATE(),@PARENTGUID_PSY from WorkflowUsersMapping WSR WHERE WSR.WorkFlowName=@Workflow_PSY AND WSR.Type='Approve'
